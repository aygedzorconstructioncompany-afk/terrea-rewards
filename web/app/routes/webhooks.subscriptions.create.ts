...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    const existing = await prisma.subscription.findFirst({ where: { shop, customerId } });
    ...
    if (productId && !existingProducts.includes(productId)) {
      ...
      existingDetails.push({ id: productId, title: productTitle, images: productImage ? [{ src: productImage }] : [] });
    }

    await prisma.subscription.upsert({
      where: { shop_customer: { shop, customerId } },
      create: {
        shop, customerId, status: "active", currentTier: "start",
        monthsActive: 0, pendingPoints: 0,
        subscriptionContractId: payload.id?.toString(),
        startedAt: new Date(), lastOrderAt: new Date(),
